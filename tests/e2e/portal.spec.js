...
test("login -> pending -> approve -> catalog -> quote", async ({ page, request, baseURL }) => {
  ...
  await request.post(`${baseURL}/auth/dev-login`, { data: { email, status: "pending" } });
  await page.goto("/");
  ...
  await adminContext.post(`${baseURL}/auth/dev-login`, { data: { email: adminEmail, role: "admin", status: "approved" } });

  // Look up the pending user's id via the admin API and approve it.
  const usersResponse = await adminContext.get(`${baseURL}/api/admin/users?status=pending`);
  const { users } = await usersResponse.json();
  const target = users.find((u) => u.email === email);
  expect(target).toBeTruthy();
  await adminContext.patch(`${baseURL}/api/admin/users/${target.id}`, { data: { status: "approved" } });
